...
#define GATTC_TAG "BLE client"
#define REMOTE_SERVICE_UUID 0x180F
#define REMOTE_NOTIFY_CHAR_UUID 0x2A19
#define PROFILE_NUM 1
#define PROFILE_A_APP_ID 0
#define INVALID_HANDLE 0
#define ESP_GATT_UUID_CHAR_CLIENT_CFG 0x2901

static const char remote_device_name[] = "sensor_data";
static bool connect = false;
static int conn_counter = 0;
static int conn_threshold = 16;
...
static write_target_t write_target = {
  .char_handle = 0,
  .conn_id = 0,
  .gatt_if = 0,
};
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event,
  ...
  case ESP_GATTC_SEARCH_CMPL_EVT:
    ...
    if (p_data->search_cmpl.searched_service_source ==
      ...
      //This will gets triggered because of the goto jump
      ...
      if (count > 0) {
        ...
        if (!char_elem_result) {
          ...
          if (status != ESP_GATT_OK) {
            ...
            write_target.char_handle = char_elem_result[0].char_handle;
            write_target.conn_id = p_data->search_cmpl.conn_id;
            write_target.gatt_if = gattc_if;
            init_finished = true;
            /*ESP_ERROR_CHECK(esp_ble_gattc_write_char(
                gattc_if, p_data->search_cmpl.conn_id,
                char_elem_result[0].char_handle, 16,
                (uint8_t *)"hallo, wie gehts", ESP_GATT_WRITE_TYPE_NO_RSP,
                ESP_GATT_AUTH_REQ_NONE));*/
          }

          /*  Every service have only one char in our 'ESP_GATTS_DEMO' demo, so
           * we used first 'char_elem_result' */
  ...
  case ESP_GATTC_READ_CHAR_EVT: {
    ESP_LOGW(GATTC_TAG, "READ EVENT");
    const struct gattc_read_char_evt_param *read_param = &p_data->read;
    if (p_data->read.status != ESP_GATT_OK) {
      ESP_LOGE(GATTC_TAG,
               "Failed to read value from characteristics, error: %x",
               p_data->read.status);
    } else {
      //NOTHING
    }
    ...
    if (p_data->reg_for_notify.status != ESP_GATT_OK) {
      ...
      if (count > 0) {
        ...
        if (!descr_elem_result) {
          ...
          /* Every char has only one descriptor in our 'ESP_GATTS_DEMO' demo, so
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event,
  ...
  case ESP_GAP_BLE_SCAN_RESULT_EVT: {
    ...
    ESP_LOGI(GATTC_TAG, "ESP_GAP_BLE_SCAN_RESULT_EVT: %d",
...
int ble_init(void *self, void *args) {
  ...
  */
  ...
  while(!init_finished && conn_counter < conn_threshold) {
    conn_counter++;
    esp_task_wdt_reset();
  ...
  if(!init_finished) {
    ESP_LOGE(GATTC_TAG, "ble connection could not be established");
  }
...
int ble_send(void *self, const char *data, int len) {
  if(init_finished) {
    int err = esp_ble_gattc_write_char(write_target.gatt_if, write_target.conn_id, write_target.char_handle, len,
                                             (uint8_t*)data,
